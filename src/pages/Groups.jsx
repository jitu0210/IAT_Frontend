...
export default function Groups() {
  ...
  const loadGroups = async () => {
    try {
      ...
      // Check if user has rated each group
      const ratingStatus = {};
  ...
  const handleJoinGroup = async (groupId) => {
    try {
      ...
      await loadGroups(); // Reload groups to reflect the change
  ...
  const handleSubmitRating = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/groups/${selectedGroup._id}/rate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      await loadGroups(); // Reload groups to reflect the new rating
  ...
  const handleRemoveRating = async (groupId) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/groups/${groupId}/rating`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || 'Failed to remove rating');
      }
    ...
    <div className="bg-gray-900 text-gray-200 min-h-screen flex flex-col">
      ...
      <section className="py-12 px-4 flex-grow">
        <div className="max-w-6xl mx-auto">
          ...
          {loading && (
            <div className="max-w-2xl mx-auto mb-6">
              <div className="bg-blue-900/30 border border-blue-700 text-blue-400 px-4 py-3 rounded-lg text-center">
                Loading...
              </div>
            </div>
          )}
            ...
            {(!selectedGroup.members || selectedGroup.members.length === 0) ? (
              ...
              <div className="space-y-3">
                {selectedGroup.members.map((member) => (
                  <div key={member.userId} className="bg-gray-700 p-3 rounded-lg">
