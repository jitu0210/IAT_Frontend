...
export default function Groups() {
  ...
  const handleSubmitRating = async () => {
    try {
      ...
      await api.post(`/groups/${selectedGroup._id}/rate`, {
  ...
  const isUserMember = (groupId) => {
    ...
    return group && group.members && group.members.some(member => member.userId === user?._id);
  };
  ...
  return (
    <div className="bg-gray-900 text-gray-200 min-h-screen flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showLiveRanking && (
          <div className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col hidden lg:flex">
            <div className="p-4 border-b border-gray-700 bg-gray-900 flex justify-between items-center">
              <h2 className="text-xl font-bold text-blue-400">Live Ranking Updates</h2>
              <button 
            ...
            <div className="flex-1 overflow-hidden relative">
              ...
              <div className="absolute top-2 right-2 flex items-center z-10">
                ...
                <span className="ml-1 text-xs text-red-400 font-medium">LIVE</span>
              </div>
                  ...
                  {newRatings.map((rating) => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <span className="font-medium text-white">{rating.groupName}</span>
                        <span className="text-xs text-gray-400">{rating.time}</span>
                      </div>
                      <div className="flex justify-between mt-1">
                        <span className="text-sm text-gray-300">Received rating:</span>
                        <span className="font-bold text-green-400">+{rating.points} pts</span>
                      </div>
                      {rating.isUser && (
                        <div className="text-xs text-blue-400 mt-1">Your rating</div>
                      )}
                    </div>
                  ))}
          ...
          <div className="max-w-6xl mx-auto">
              ...
              <div className="mb-6 p-4 bg-gray-800 rounded-xl border border-gray-700 shadow-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-blue-400 font-medium">{user.name || user.email}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mb-8">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-blue-400">Groups Ranking</h2>
                <div className="text-sm text-gray-400">
                  {sortedGroups.length} group{sortedGroups.length !== 1 ? 's' : ''} total
                </div>
              </div>
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
                  {sortedGroups.map((group, index) => {
                    ...
                    return (
                      <div
                        key={group._id}
                        className="bg-gray-800 rounded-xl p-6 flex flex-col border border-gray-700 hover:border-blue-400/30 transition-all shadow-lg hover:shadow-xl transform hover:-translate-y-1"
                      >
                        {/* Ranking badge */}
                        <div className={`absolute -top-3 -left-3 w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg ${
                        ...
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <div className="text-right">
                            <div className="text-2xl font-bold text-green-400">
                              ...
                              <span className="text-sm text-gray-400">/240</span>
                            </div>
                            <div className="text-sm text-gray-400">
                        ...
                        <div className="mb-4">
                          <div className="flex justify-between text-xs text-gray-400 mb-1">
                            ...
                            <span>{Math.round((stats.totalRating / 240) * 100)}%</span>
                          </div>
                          <div className="w-full bg-gray-700 rounded-full h-2">
                            <div 
                              className="bg-gradient-to-r from-blue-500 to-purple-500 h-2 rounded-full" 
                              style={{ width: `${(stats.totalRating / 240) * 100}%` }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 text-sm text-gray-400 space-y-2">
                          {[
                            ...
                            { label: "Presentation", value: stats.avgPresentation },
                            { label: "Content", value: stats.avgContent },
                            { label: "Useful for Company", value: stats.avgHelpfulForCompany },
                            { label: "Useful for Interns", value: stats.avgHelpfulForInterns },
                            { label: "Participation", value: stats.avgParticipation },
                          ].map((item, i) => (
                            <div key={i} className="flex justify-between items-center">
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="font-medium w-10 text-right">{item.value.toFixed(1)}</span>
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          <div className="bg-gray-800 rounded-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-gray-700">
                <p className="font-bold text-lg text-center text-green-400">Total: {calculateTotal()} / 240</p>
              </div>
            ...
            {(!selectedGroup.members || selectedGroup.members.length === 0) ? (
              ...
              <div className="space-y-3">
                {selectedGroup.members.map((member) => (
                  <div key={member.userId} className="bg-gray-700 p-4 rounded-lg border border-gray-600">
